...
3. ~~**Keyboard shortcuts (⌘L/⌘T/⌘W/⌘R/⌘[ ⌘]) work while the pane's chrome has focus.**~~ **Resolved by Task 10:** the app now builds its own native menu bar whose key equivalents cover every shortcut, so a chord the focused page ignores reaches the menu and is emitted back to the main webview as a synthetic keydown — the shortcuts work with the page focused, as originally specified.
...
5. **The capability is scoped with `webviews: ["main"]`,** and `build.rs` generates an app ACL manifest so a child webview cannot invoke any command. `windows: ["main"]` would have handed the same IPC to every browser tab in that window; without the generated manifest, a local page would bypass the ACL entirely. Both are pinned by tests.
6. **The browser profile uses `data_store_identifier`,** not `data_directory` — the latter looks like the knob for a separate profile but is a no-op on macOS. A fixed UUID names the tabs' own `WKWebsiteDataStore` on macOS 14+; on older macOS the identifier is ignored and the tabs share the app's WebKit store.
7. **Block-list chunks ship gzipped** (`N.json.gz`, ~1 MB each against ~20 MB of JSON) and are inflated in Rust, on the command's worker thread rather than the main thread.
8. **Toasts sit top-right, over the pane's DOM chrome.** A native child webview always paints over the app's DOM, so a toast above the page region would be invisible whatever the column layout is; top-right keeps them over the tab strip and address bar.
9. **The ⋯ menu counts as an overlay:** it is DOM, so the native page would cover it. The page hides while the menu is open, and the cover says why.
