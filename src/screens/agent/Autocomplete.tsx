...
export function Autocomplete({
  ...
  if (items.length === 0) {
    return (
      <div
        data-composer-menu
        className="absolute bottom-full left-0 z-(--z-popover) mb-1.5 w-[340px] max-w-[calc(100%-8px)] rounded-lg border border-border-strong bg-surface-overlay px-3 py-2.5 [box-shadow:var(--shadow-overlay)]"
      >
  ...
  return (
    <div
      ...
      // scrollbar came from
      className="absolute bottom-full left-0 z-(--z-popover) mb-1.5 flex max-h-[360px] w-[380px] max-w-[calc(100%-8px)] flex-col overflow-y-auto overflow-x-hidden rounded-lg border border-border-strong bg-surface-overlay p-1 [box-shadow:var(--shadow-overlay)]"
    >
      {items.map((it, i) => {
          ...
          <div key={it.id} className="flex flex-col">
            ...
            >
              ...
              <span className="flex w-full min-w-0 items-center gap-2">
                ...
                {it.badge && (
                  <span className="ml-auto shrink-0 rounded-xs bg-fill-subtle px-1.5 text-[10px] text-text-subtle">
                    {it.badge}
                  </span>
                )}
              </span>
